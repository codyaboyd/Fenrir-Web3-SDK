...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function init_template(){
		...
		setTimeout(function(){if(menuHider){menuHider[0].classList.remove('menu-active');}},50)
        ...
        highlightData.forEach(el => el.addEventListener('click', e =>{
            ...
            loadHighlight.href = 'UI/highlights/highlight_' + highlight +'.css';
        ...
        if(rememberHighlight){
            ...
            loadHighlight.className = "page-highlight";
            loadHighlight.type = "text/css";
            loadHighlight.href = 'UI/highlights/highlight_' + rememberHighlight +'.css';
            ...
            loadHighlight.className = "page-highlight";
            loadHighlight.type = "text/css";
            loadHighlight.href = 'UI/highlights/highlight_' + defaultHighlight[1] +'.css';
            if(!document.querySelectorAll('.page-highlight').length){
                document.getElementsByTagName("head")[0].appendChild(loadHighlight);
...
//ThemeInk
function inkerPrim(colors) {
	console.log("changing theme to " + colors);
	$(".themeInk").css("background-image", 'linear-gradient(to right top, ' + colors +')');
}
function inkerSec(colors) {
	console.log("changing theme to " + colors);
	$(".themeInk2").css("background-image", 'linear-gradient(to right top, ' + colors +')');
}
function inkerTert(colors) {
	console.log("changing theme to " + colors);
	$(".themeInk3").css("background-image", 'linear-gradient(to right top, ' + colors +')');
}

//CoinGecko-Price
var coinGeckoPrice = null;

async function getPrice(currency,compared) {
	let fetchUrl = "https://api.coingecko.com/api/v3/simple/price?ids="+currency+"&vs_currencies="+compared;
		await $.getJSON(fetchUrl, function (data) {
		coinGeckoPrice = (data[currency][compared]);
	})
}
